...
我承认有标题党嫌疑，300行代码也只能实现最基本的“不可变数据类型”，但是完整的功能是在这基础上扩展而来，本文以immerJs的源码实现为底，围绕着immerJs的原理来编写。
...
先看看`Proxy`怎么用的，`Proxy`第二个参数是一个对象，可以传入 `get`，`set`，`deleteProperty`等“陷阱”，当对对象进行增，删，改，查的时候，都会触发相应的方法
